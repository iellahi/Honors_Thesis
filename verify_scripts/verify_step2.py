"""
Step 2: Symbolic verification of the (1,1) symmetric development state derivation.
...
Thesis: Section 4.1 and Appendix A.1 (Bellman -> FOC -> equilibrium quadratic).
"""
import sympy as sp
...
V_reference = (W * (2 * P_i - P_i * P_j) - c * x_i**2) / (2 * (P_i + P_j - P_i * P_j))
print("PASS" if sp.simplify(V_isolated - V_reference) == 0 else "FAIL")
...
foc_reference = -c * x_i + lam * (W - (P_j + beta * P_i) * W / 2
                             - (1 + beta) * V + (P_j + beta * P_i) * V)
print("PASS" if sp.simplify(foc_lhs_minus_rhs - foc_reference) == 0 else "FAIL")

print("=" * 70)
print("CHECK 4: Symmetric FOC  c x = lam W - M V - M^2 x (W/2 - V),  M = lam(1+beta)")
print("=" * 70)
foc_sym = foc_reference.subs([(x_i, x), (x_j, x)])
foc_reference_M = -c * x + lam * W - M * V - M**2 * x * (W / 2 - V)
print("PASS" if sp.simplify(sp.expand(foc_sym) - sp.expand(foc_reference_M)) == 0 else "FAIL")

print("=" * 70)
print("CHECK 5: V from FOC:  V = (c x - lam W + M^2 x W/2) / (M(Mx - 1))")
print("=" * 70)
V_from_foc = sp.solve(sp.Eq(foc_reference_M, 0), V)[0]
V_foc_reference = (c * x - lam * W + M**2 * x * W / 2) / (M * (M * x - 1))
print("PASS" if sp.simplify(V_from_foc - V_foc_reference) == 0 else "FAIL")

print("=" * 70)
print("CHECK 6: V from Bellman under symmetry: V = (W(2M - M^2 x) - c x)/(2M(2 - Mx))")
print("=" * 70)
V_bell_sym = sp.simplify(V_isolated.subs([(x_i, x), (x_j, x)]))
V_bell_reference = (W * (2 * M - M**2 * x) - c * x) / (2 * M * (2 - M * x))
print("PASS" if sp.simplify(V_bell_sym - V_bell_reference) == 0 else "FAIL")
...
quad_reference_M = M * c * x**2 - (3 * c + 2 * lam * M * W - M**2 * W) * x + 2 * W * (2 * lam - M)
# numer should be proportional to quad_reference_M
ratio = sp.simplify(sp.expand(numer) / sp.expand(quad_reference_M))
print("Numerator / reference quadratic =", ratio)
...
print("PASS" if sp.simplify(sp.expand(quad_reference_M) - sp.expand(quad_final)) == 0 else "FAIL")

print("=" * 70)
print("CHECK 9: Discriminant Delta = 9c^2 - 2c lam^2 W(1-b^2) + lam^4 W^2 (1-b^2)^2")
print("=" * 70)
Delta = sp.expand(B**2 - 4 * A * C)
Delta_reference = 9 * c**2 - 2 * c * lam**2 * W * (1 - beta**2) + lam**4 * W**2 * (1 - beta**2)**2
print("PASS" if sp.simplify(Delta - sp.expand(Delta_reference)) == 0 else "FAIL")
...
print("=" * 70)
x_star = (3 * c + lam**2 * W * (1 - beta**2) - sp.sqrt(Delta_reference)) / (2 * c * lam * (1 + beta))
...
print("=" * 70)
V_final_reference = c * x_star / (2 * lam * (1 + beta)) + beta * W / (1 + beta)
V_check_foc = sp.simplify(V_from_foc.subs(x, x_star) - V_final_reference)
V_check_bell = sp.simplify(V_bell_sym.subs(x, x_star) - V_final_reference)
...
V_num = float(V_final_reference.subs(subs_num))
...
x_plus = (3 * c + lam**2 * W * (1 - beta**2) + sp.sqrt(Delta_reference)) / (2 * c * lam * (1 + beta))
