...
Thesis: Appendix D.2 and D.3 (master quadratic, Lemma 4; root selection, Lemma 5).
"""
import sympy as sp
...
print("="*70)
print("N1: cleaned form is proportional to the raw polynomial (ratio free of x)")
print("="*70)
...
# The claim is proportionality in x: the two polynomials in x agree up to a
# factor that carries no x. That factor need not be a pure number -- here it is
# 1/(1+b0) -- so test that the ratio is free of x and nonzero.
ok = (x not in ratio.free_symbols) and sp.simplify(ratio) != 0
