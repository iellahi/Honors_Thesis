...
Task 4 -- comparative statics within the assumption region.
  ...
  (c) dx*/dlam total vs direct (V11 frozen): rare sign reversals -> map + share.
...
Thesis: Sections 4.2 and 5.5; the laggard BR slope identity is Appendix C.4.
"""
...
log(""); log(f"figure written: {os.path.relpath(f1, THESIS)}")
...
log(f"figure written: {os.path.relpath(f2, THESIS)}   (slice reversal cells: {n_flip}/{n_cells})")
...
log(f"figure written: {os.path.relpath(f3, THESIS)}")
