...
Task 2 -- asymmetric states (1,0)/(0,1).
  ...
  (b) Confirm Prop. 3 (leader dominance), proven for mu <= lam, on the mu = lam
      slice of the scan; expect 0 violations.
...
Thesis: Section 4.2 and Appendix C.1-C.3; leader dominance is Proposition 3.
"""
...
# ==========================================================================
log("="*74)
log("PART A: asymmetric fixed point -- residuals, Prop. 3, value ordering")
log("="*74)
random.seed(11)
N = 60000
max_res = 0.0
...
log("")
log(f"[Prop. 3] mu = lam : z* > x* fails {fail_sym} / {n_sym}   (expect 0)")
...
axA.plot([0.02, 0.49], [0.02, 0.49], "k--", lw=1.4, label=r"$\mu=\lambda$;  Prop. 3 proven below")
...
axA.text(0.13, 0.40, r"$z^*\leq x^*$"+"\n(Prop. 3 fails)", color="white", ha="center", va="center",
...
axB.set_title(r"(B) Prop. 3 failure share by $\mu/\lambda$   (200k draws)")
...
log("")
log(f"figure written: {os.path.relpath(outpng, THESIS)}")
