...
Produced elsewhere: prop2_frontier.png (Task 2), laggard_BR.png (Task 4).
...
Thesis: figures for Sections 4.1-4.3.
"""
import math, os
import numpy as np
import matplotlib
...
a2.set_title(r"(B) tied-state value $V_{11}$ rises (Prop. 2)")
...
log(f"figure: {os.path.relpath(f1, THESIS)}")
...
log(f"figure: {os.path.relpath(f2, THESIS)}")
...
log(f"figure: {os.path.relpath(f3, THESIS)}  (beta11={B11_F} for visibility; monotone but modest)")
