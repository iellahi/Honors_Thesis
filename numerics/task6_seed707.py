"""
Task 6 -- consolidated verification on a second fresh seed (707).
Identical battery to task6_summary.py (seed 202); the point is that an
independent draw reproduces every result.
...
Solvers ported from verify_step5b.py.
Outputs: numerics/task6_seed707.txt  (raw confirmation tables, second seed)

Thesis: Section 5.2, second seed.
"""
...
# ==========================================================================
log("="*78); log("PART 1/2: THEOREM cross-checks + NUMERICAL claims  (seed 707, N=60000, A1/A2/A2')")
log("="*78)
random.seed(707)
N = 60000
V = {k: 0 for k in [   # violation counters (theorems -> expect 0)
    "T1_Delta_pos","T1_Delta_form","T2_sub_interior","T2_add_super","T3_key_ineq",
    "T4_prop1_signs","T5_prop2_tiedvalue_signs","T6_ordering","T7_prop3_sym","T8_BRslope","T9_prop4_signs",
    "T11_BRsolve"]}
...
for i in range(N):
    ...
    # --- Prop. 2 signs: V11 wrt (b1,W,c,lam) = (+,+,+,-) ---
    ...
    if sv != (1,1,1,-1): V["T5_prop2_tiedvalue_signs"] += 1
    # --- asymmetric solve ---
    ...
    if mu == lam and not (z > x): V["T7_prop3_sym"] += 1
    # --- Prop. 3 asym share (numerical) ---
    if mu != lam:
        num["asym_tot"] += 1
    ...
    if D0 <= 0: num["D0_bad"] += 1
        ...
        # Prop. 4: dx00 wrt (V10,V11,V01) = (+,+,-)  (partials, other V's fixed)
        e = 1e-6
        ...
        if (s10,s11,s01) != (1,1,-1): V["T9_prop4_signs"] += 1
...
labels = {
 ...
 "T5_prop2_tiedvalue_signs":"Prop. 2 signs (V11: +,+,+,-)","T6_ordering":"value ordering 0<=V01<=V11<W/2<=V10<=W",
 "T7_prop3_sym":"Prop. 3 (z>x at mu=lam)","T8_BRslope":"BR slope sign = sign(N)",
 "T9_prop4_signs":"Prop. 4 signs (x00: +,+,-)","T11_BRsolve":"additive-root BRs solve G_L,G_F=0"}
# Rows checked by central finite differences.  A near-zero true derivative cannot be
# signed in float, so a nonzero count here is a FLAG to adjudicate, not a failure:
# task6_diagnose_seed707.py re-solves every flagged case at 50 digits.  Any other row
# is an exact check, so a nonzero count there is a genuine FAIL.
FD_ROWS = {"T4_prop1_signs", "T5_prop2_tiedvalue_signs", "T9_prop4_signs"}
allpass, nflag = True, 0
for k in ["T1_Delta_pos","T1_Delta_form","T2_sub_interior","T2_add_super","T3_key_ineq",
          "T4_prop1_signs","T5_prop2_tiedvalue_signs","T6_ordering","T7_prop3_sym","T8_BRslope",
          "T9_prop4_signs","T11_BRsolve"]:
    if V[k] == 0:
        status = "PASS"
    elif k in FD_ROWS:
        status = f"FLAG ({V[k]})"; nflag += V[k]
    else:
        status = f"FAIL ({V[k]})"; allpass = False
...
if nflag:
    log(f"  => {nflag} finite-difference FLAGs, 0 real violations. FLAGs mark near-zero")
    log( "     derivatives, adjudicated at 50 digits by task6_diagnose_seed707.py.")
...
random.seed(913); n=40000; corner=0; tot=0
for _ in range(n):
...
# (ii) draw mu != lam -> Prop. 3's hypothesis (mu <= lam) can fail
random.seed(914); n=40000; fail=0; tot=0
for _ in range(n):
    ...
    tot+=1
    if not (z>x): fail+=1
log(f"(ii) relax mu=lam: Prop. 3 (z>x) does not hold in {fail}/{tot} = {100*fail/tot:.1f}% "
    f"[Prop. 3 is proven for mu <= lam; every failure here has mu > lam]")
# (iii) relax (A2') muW>c while keeping lamW<=c (needs mu>lam): check (0,0) claims
random.seed(915); n=60000; tot=0; D0bad=0; x00bad=0; mx00bad=0
for _ in range(n):
...
with open(os.path.join(NUM, "task6_seed707.txt"), "w") as f:
    f.write("\n".join(log_lines) + "\n")
...
with open(os.path.join(NUM, "task6_seed707.txt"), "w") as f:
    f.write("\n".join(log_lines) + "\n")
print("\n[written]", os.path.join(NUM, "task6_seed707.txt"))
