...
Solvers ported from verify_scripts/verify_step5b.py (reference).  PNG 300 dpi.
  ...
  (a) sqrt(2)-frontier: dx*/dV11 > 0 is proven wherever x* < sqrt(2) z* (R2).
      ...
      share of Prop. 3 failure draws (z* <= x*) still inside the proven region;
...
Thesis: Section 5.5; the sqrt(2) condition is Proposition 5, Appendix C.5.
"""
...
# ==========================================================================
# PART (a) -- sqrt(2)-frontier vs Prop. 3 failure region   seed 41, N=60000
# ==========================================================================
log("="*74)
log("PART A: sqrt(2)-frontier (proven-transmission region) vs Prop. 3 failures")
log("="*74)
random.seed(41)
N = 60000
bins = [0.0, 0.5, 0.8, 1.0, 1.25, 1.6, 2.0, 3.0, 1e9]
btot  = [0]*(len(bins)-1)      # draws per bin
bfail = [0]*(len(bins)-1)      # z* <= x*  (Prop. 3 failure)
...
log(f"Prop. 3 failures (z*<=x*):                  {n_fail}/{n_all} = {100*n_fail/n_all:.2f}%")
log(f"Prop. 3 failures still inside sqrt2 region: {n_fail_sq2}/{n_fail} = {100*n_fail_sq2/max(n_fail,1):.2f}%")
log("")
...
log(f"grid (task2 config): max x*/(sqrt2 z*) = {np.nanmax(RATIO):.4f} "
    f"(< 1 on this grid: proven-transmission region covers the whole grid, incl. all "
    f"{int((SIGN < 0.5).sum())} Prop. 3 failure cells)")
...
ax.plot([], [], color="white", lw=2.2, label=r"$z^*=x^*$ (Prop. 3 frontier)")
...
ax.plot([0.02, 0.49], [0.02, 0.49], "k--", lw=1.2, label=r"$\mu=\lambda$;  Prop. 3 proven below")
ax.set_xlim(0.02, 0.49); ax.set_ylim(0.02, 0.49)
ax.set_xlabel(r"leader rate $\lambda$"); ax.set_ylabel(r"laggard rate $\mu$")
ax.set_title(r"margin to the $x^*=\sqrt{2}z^*$ frontier: ratio $<1$ on this grid,"
...
log(f"figure written: {os.path.relpath(out1, THESIS)}")
...
log(f"figure written: {os.path.relpath(out2, THESIS)}")
